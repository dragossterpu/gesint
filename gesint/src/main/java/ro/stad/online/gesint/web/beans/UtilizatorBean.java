...
public class UtilizatorBean implements Serializable {
	...
	public void activareUtilizator(final Utilizator usu) {
		try {
			...
			FacesUtilities.setMesajConfirmareDialog(FacesMessage.SEVERITY_INFO, RegistruEnum.MODIFICARE.getDescriere(),
					Constante.OKELIMINAREMESAJ);
			log.info("activareUtilizator: userul:".concat(utilizator.getNumeComplet()).concat(" a fost activat de: ")
			...
			this.regActividadService.inregistrareRegistruActivitate(descriere, RegistruEnum.ACTIVARE.getName(),
			...
			this.regActividadService.salveazaRegistruEroare(descriere, SectiuniEnum.USERS.getDescriere(), e);
			log.error(descriere);
		}
	}

	/**
	 * Metodă folosită pentru a returna o listă a localităților care aparțin unui judet. Acesta este folosit pentru a
	 * reîncărca lista localităților în funcție de judetul selectat.
	...
	public List<Localitate> actualizareLocalitati(final String prov) {
		...
		this.judet = this.judetService.findById(prov);
		if (this.judet != null) {
			try {
	...
	public List<Localitate> actualLocalitati(final String jude) {
		...
		if (this.judet != null) {
			try {
				this.localitati = this.localitateService.findByJudet(this.judet);
				log.info("S-a obținut cu succes lista de localități care aparțin județului  ".concat(judet.getNume()));
			}
			catch (final DataAccessException e) {
				FacesUtilities.setMesajConfirmareDialog(FacesMessage.SEVERITY_ERROR, RegistruEnum.EROARE.getDescriere(),
						Constante.DESCEROAREMESAJ);
	...
	public void cautareUtilizator() {
		...
		final String mesajModificare = mesajCautareUtilizator(filtruUtilizator);
	...
	public void cautareUtilizatorLocal() {
		...
		if (this.filtruEchipa.getIdFunctia() == null) {
			final List<Functie> lista = incarcamToateFunctileLocale();
			this.filtruEchipa.setListaFunctii(lista);
		}
		try {
		...
		if (this.functieIdUser != usu.getTeam().getId()) {
			...
			try {
				...
				final String descriere = "Utillizatorului: ".concat(usu.getNumeComplet())
						.concat(" ia fost modificată cu succes funcția ").concat(nouaFunctie.getNume()).concat(" de: ")
						.concat(this.user.getUsername());
				this.regActividadService.inregistrareRegistruActivitate(descriere, RegistruEnum.ELIMINARE.getName(),
						SectiuniEnum.USERS.getName(), user);
				log.info(descriere);
			}
			catch (final DataAccessException e) {
	...
	public void eliminareUtilizator(final Utilizator usu) {
		try {
			...
			this.utilizatorService.save(this.utilizator);
			FacesUtilities.setMesajConfirmareDialog(FacesMessage.SEVERITY_INFO, RegistruEnum.ELIMINARE.getDescriere(),
					Constante.OKELIMINAREMESAJ);
			log.info("eliminareUtilizator: userul:".concat(utilizator.getNumeComplet()).concat(" a fost eliminat de: ")
					.concat(utilitati.getUtilizatorLogat().getUsername()));
			final String descriere = Constante.UTILIZATORUL.concat(usu.getNumeComplet())
		...
		try {
			...
			if (usu.getTeam().getId() == NumarMagic.NUMBERTWENTYONELONG) {
				...
				if (presedinte != usu && presedinte != null) {
					validaPresedinte = false;
					this.mesaj = "Există un președinte al organizației ".concat(presedinte.getCodJudet().getNume())
	...
	private void extractRecuperaPrimaLocalitate(final String local) {
		if (!this.utilizatorModificat.getLocalitate().getNume().equals(local)) {
			...
			if (Constante.SPATIU.equals(local) && local == null) {
				...
				this.localitateSelectata = this.localitateService.localidadByNumeIgnoreCaseAndJudet(local, this.judet);
			}
		}
		else {
	 ...
	 * @param valida
	 */
	private void extractUserValida(final String mesajModificare, final String valida) {
		if (validar(valida)) {
			if (!this.eliminat) {
				this.utilizator.setDateDeleted(null);
			}
			...
			if (this.utilizator.getRank() == null && this.utilizator.getTeam().getId() != NumarMagic.NUMBERTHIRTYL) {
				if (esteLocal) {
					List<Functie> lista = new ArrayList<>();
					lista = incarcamToateFunctileLocale();
					this.filtruEchipa.setListaFunctii(lista);
					this.listUsersLocal = this.utilizatorService.findByJudetSiEchipa(this.judet, lista);
			...
			this.regActividadService.inregistrareRegistruActivitate(descriere, RegistruEnum.MODIFICARE.getName(),
					SectiuniEnum.USERS.getName(), user);
		}
		else {
	...
	/**
	 * Metodă folosită pentru a dechide formularul de înregistrare al utilizatorilor noi, inițializând tot ceea ce este
	 * necesar pentru afișarea corectă a paginii (enums, utilizator nou,..etc).
	...
	/**
	 * Metodă folosită pentru a obține imaginea pentru a previzualiza în cazul în care documentul este de tip imagine..
	...
	public void incarcareImagine(final FileUploadEvent event) throws IOException {
		...
		this.utilizator = this.utilizatorService
				.incarcareImaginaFaraStocare(IOUtils.toByteArray(uFile.getInputstream()), this.utilizator);
	...
	public void inregistrareUtilizator(final Utilizator usu) {
		try {
			...
			if (validar(valida)) {
				...
				this.utilizator.setPassword("$2a$10$tDGyXBpEASeXlAUCdKsZ9u3MBBvT48xjA.v0lrDuRWlSZ6yfNsLve");
				this.utilizatorService.save(this.utilizator);
				...
				this.regActividadService.inregistrareRegistruActivitate(descriere, RegistruEnum.INREGISTRARE.getName(),
						SectiuniEnum.USERS.getName(), this.user);
			}
			else {
				FacesUtilities.setMesajConfirmareDialog(FacesMessage.SEVERITY_ERROR, RegistruEnum.EROARE.getDescriere(),
						this.mesajEroare);
				log.info("inregistrareUtilizator: membrul nou: "
						.concat(utilizator.getNume().concat(Constante.SPATIUMARE)).concat(utilizator.getPrenume())
						.concat(" nu a fost validat"));
	...
	private String mesajModificareUtilizator(final Utilizator usu) {
		...
		if (!this.utilizatorModificat.getNume().equals(usu.getNume())) {
			mesajModificare = "Numele anterior : ".concat(this.utilizatorModificat.getNume().concat(Constante.VIRGULA));
		}
		if (!this.utilizatorModificat.getPrenume().equals(usu.getPrenume())) {
			mesajModificare = mesajModificare
					+ " Prenumele anterior : ".concat(this.utilizatorModificat.getPrenume().concat(Constante.VIRGULA));
		}
		if (!this.utilizatorModificat.getIdCard().equals(usu.getIdCard())) {
			mesajModificare = mesajModificare
					+ " CNP-ul anterior : ".concat(this.utilizatorModificat.getIdCard().concat(Constante.VIRGULA));
		}
		if (!this.utilizatorModificat.getTeam().equals(usu.getTeam())) {
			mesajModificare = mesajModificare + " Funcția anterioară : "
					.concat(this.utilizatorModificat.getTeam().getDescriere().concat(Constante.VIRGULA));
		}
		if (!this.utilizatorModificat.getRole().equals(usu.getRole())) {
			mesajModificare = mesajModificare + " Rolul anterior : "
					.concat(this.utilizatorModificat.getRole().getDescription().concat(Constante.VIRGULA));
		}
		if (!this.utilizatorModificat.getPhone().equals(usu.getPhone())) {
			mesajModificare = mesajModificare
					+ " Telefonul anterior : ".concat(this.utilizatorModificat.getPhone().concat(Constante.VIRGULA));
		}
		if (!this.utilizatorModificat.getEmail().equals(usu.getEmail())) {
			mesajModificare = mesajModificare
					+ " Email-ul anterior : ".concat(this.utilizatorModificat.getEmail().concat(Constante.VIRGULA));
		}
		if (!this.utilizatorModificat.getAdresa().equals(usu.getAdresa())
			...
			mesajModificare = mesajModificare
					+ " Adresa anterioară : ".concat(this.utilizatorModificat.getAdresa().concat(Constante.VIRGULA));
		}
		...
		if (!this.utilizatorModificat.getEducatie().equals(usu.getEducatie())
			...
			mesajModificare = mesajModificare + " Educație anterioară : "
					.concat(this.utilizatorModificat.getEducatie().getDescription().concat(Constante.VIRGULA));
		}
		...
		if (!this.utilizatorModificat.getSex().equals(usu.getSex()) && this.utilizatorModificat.getSex() != null) {
			mesajModificare = mesajModificare + " Sexul anterior : "
					.concat(this.utilizatorModificat.getSex().getDescription().concat(Constante.VIRGULA));
		}
		if (!this.utilizatorModificat.getStatutCivil().equals(usu.getStatutCivil())
				&& this.utilizatorModificat.getStatutCivil() != null) {
			mesajModificare = mesajModificare + " Starea civilă anterioară : "
					.concat(this.utilizatorModificat.getStatutCivil().getDescription().concat(Constante.VIRGULA));
		}
		if (!this.utilizatorModificat.getValidat().equals(usu.getValidat())) {
			...
			mesajModificare = mesajModificare + " Starea anterioară : ".concat(valida.concat(Constante.VIRGULA));
		}
		if (!this.utilizatorModificat.getCodJudet().equals(usu.getCodJudet())
			...
			mesajModificare = mesajModificare + " Județul/Organizația anterioară : "
					.concat(this.utilizatorModificat.getCodJudet().getNume().concat(Constante.VIRGULA));
		}
		if (this.utilizatorModificat.getLocalitate() != null
				&& !this.utilizatorModificat.getLocalitate().equals(usu.getLocalitate())) {
			mesajModificare = mesajModificare
					+ " Localitatea/Sectorul anterior : ".concat(this.utilizatorModificat.getLocalitate().getNume());
		}
	...
	public void modificareUtilizator(final Utilizator usu, final String jude, final String local) {
		...
		final String mesajModificare = mesajModificareUtilizator(usu);
	...
	public void nouaLocalitate(final String nume, final String jude, final TipLocalitateEnum tipLocalitate,
		...
		try {
			...
			existeLocalidad = this.localitateService.existeByNumeIgnoreCaseAndJudet(nume.trim(), this.judet);
		}
		catch (final DataAccessException e) {
			...
			log.error("nouaLocalitate: S-a produs o erroare în căutarea localitații ".concat(String.valueOf(e)));
		...
		if (existeLocalidad) {
			FacesUtilities.setMensajeInformativo(FacesMessage.SEVERITY_ERROR,
					...
					null, "inputNume");
			log.info(
					"nouaLocalitate: Acțiunea nu este permisă. Există deja o localitate care aparține aceluiași județ cu același nume");
		}
		else {
			...
			try {
				...
				FacesUtilities.setMensajeInformativo(FacesMessage.SEVERITY_ERROR,
						"Eroare în salvarea localității. ".concat(Constante.DESCEROAREMESAJ), null, "inputNume");
	...
	public String restabilireParola() {
		try {
			...
			FacesUtilities.setMesajConfirmareDialog(FacesMessage.SEVERITY_ERROR, Constante.PAR,
					"A apărut o eroare în regenerarea sau trimiterea parolei. ".concat(Constante.DESCEROAREMESAJ));
	...
	/**
	 * Metodă folosită pentru controlul modificării filei grupului de date (Utilizatori, Conducere locala sau Conducere
	 * centrala).
	...
	/**
	 * Metodă folosită pentru a valida datelor unui utilizatorla inregistrare 1. Numele de utilizator nu se repetă 2.
	 * CNP nu se repetă 3. CNP valabil
	 * @return boolean
	 */
	...
	private boolean valideazaAn() {
		...
		if (this.utilizator.getIdCard().length() > NumarMagic.NUMBERSEVEN) {
			...
			final String luna = this.utilizator.getIdCard().substring(NumarMagic.NUMBERTHREE, NumarMagic.NUMBERFIVE);
			final String zi = this.utilizator.getIdCard().substring(NumarMagic.NUMBERFIVE, NumarMagic.NUMBERSEVEN);
		...
		if (!StringUtils.isEmpty(this.utilizator.getIdCard()) && this.utilizator.getIdCard() != null) {
			try {
				if (cnpCorect()) {
					...
					log.info(
							"valideazaCnpUnic: Datele introduse pentru validarea cnp-ului nu sunt corecte. Verificați aceste date și încercați din nou.");
				}
			}
			catch (final DataAccessException e) {
				FacesUtilities.setMesajConfirmareDialog(FacesMessage.SEVERITY_ERROR, RegistruEnum.EROARE.getDescriere(),
