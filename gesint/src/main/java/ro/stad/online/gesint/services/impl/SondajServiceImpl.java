...
public class SondajServiceImpl implements SondajService, Serializable {
	...
	public List<Sondaj> cautareSondajeFinalizate() {
		try {
			...
			UtilitatiCriteria.setConditieCriteriaEgalitateEnum(SituatieSondajEnum.ACTIV, crit, Constante.ACTIV);
			...
			if ((this.session != null) && this.session.isOpen()) {
				try {
					...
					FacesUtilities.setMesajConfirmareDialog(FacesMessage.SEVERITY_ERROR,
							RegistruEnum.EROARE.getDescriere(), Constante.DESCEROAREMESAJ);
					log.error(
							RegistruEnum.EROARE.getDescriere().concat(" a aparut o eroare la accesarea bazei de date"));
				}
			}
		}
	}
	...
	public List<Sondaj> cautareSondajePentruActivare() {
		try {
			this.session = this.sessionFactory.openSession();
			final Criteria crit = this.session.createCriteria(Sondaj.class);
			UtilitatiCriteria.setConditieCriteriaDataMaiMica(new Date(), crit, "dataIncepere");
			UtilitatiCriteria.setConditieCriteriaEgalitateEnum(SituatieSondajEnum.NEINCEPUT, crit, Constante.ACTIV);
			crit.setResultTransformer(Criteria.DISTINCT_ROOT_ENTITY);
			return crit.list();
		}
		finally {
			if ((this.session != null) && this.session.isOpen()) {
				try {
					...
					FacesUtilities.setMesajConfirmareDialog(FacesMessage.SEVERITY_ERROR,
							RegistruEnum.EROARE.getDescriere(), Constante.DESCEROAREMESAJ);
					log.error(
							RegistruEnum.EROARE.getDescriere().concat(" a apărut o eroare la accesarea bazei de date"));
				}
			}
		}
	}
